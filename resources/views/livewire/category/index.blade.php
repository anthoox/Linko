                ...
                <div class="px-4 py-8">
                    <div class="flex w-full flex-col gap-1 text-on-surface dark:text-on-surface-dark">
                        ...
                        <input id="name" wire:model="name" type="text"
                            ...
                            name="name" placeholder="Nombre categoría" />
                        @error('name')
...
<p class="mb-2 text-sm text-on-surface dark:text-on-surface-dark">
    O sube tu propio icono (opcional)
        ...
        dark:border-outline-dark dark:bg-surface-dark-alt/50 dark:text-on-surface-dark dark:file:bg-linko-purple dark:focus-visible:outline-linko-purple"
                        ...
                        <div wire:loading wire:target="icon" class="text-xs text-blue-500 mt-1">
                            Procesando imagen...
                        </div>
                        ...
                        @if ($icon && !$errors->has('icon'))
                            <div class="mt-2 text-xs text-green-600 flex items-center gap-1">
                                ...
                                Listo: {{ $icon->getClientOriginalName() }}
                            </div>
                        @endif

                        @if ($editingCategoryId && $currentIcon && !$icon)
                            <p class="mt-2 text-xs text-on-surface/60 dark:text-on-surface-dark/60">
                                Esta categoría ya tiene un icono guardado. Si eliges otro archivo, se reemplazará.
                            </p>
                        @endif

                        @error('icon')
                            <span class="text-xs text-start text-red-500 mt-1 font-medium">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div
                    ...
                    @if($editingCategoryId)
                        <button type="button" wire:click="confirmDeleteCategory({{ $editingCategoryId }})"
                    ...
                    <button x-on:click="modalIsOpen = false" type="button"
                        class="whitespace-nowrap rounded-radius px-4 py-2 text-center text-sm font-medium tracking-wide text-on-surface transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:opacity-100 active:outline-offset-0 dark:text-on-surface-dark dark:focus-visible:outline-primary-dark">Cancelar</button>
                    <button type="submit"
    ...
    <div
        class="mt-4 overflow-hidden w-full overflow-x-auto rounded-radius border border-outline dark:border-outline-dark">
            ...
            <tbody class="divide-y divide-outline dark:divide-outline-dark">
                @forelse($categories as $category)
                    <tr>
                        ...
                        <td class="p-4">
                            <button type="button" type="button" wire:click="editCategory({{ $category->id }})"
                                x-on:click="modalIsOpen = true"
                                class="whitespace-nowrap rounded-radius bg-transparent p-0.5 hover:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 active:opacity-100 active:outline-offset-0">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="size-6">
                                    <path fill="none" stroke="#64748B" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="2"
                                        d="m14 6l2.293-2.293a1 1 0 0 1 1.414 0l2.586 2.586a1 1 0 0 1 0 1.414L18 10m-4-4l-9.707 9.707a1 1 0 0 0-.293.707V19a1 1 0 0 0 1 1h2.586a1 1 0 0 0 .707-.293L18 10m-4-4l4 4" />
                                </svg>
                            </button>
                        </td>
                        <td class="p-4">
                            <button type="button" wire:click="confirmDeleteCategory({{ $category->id }})"
                                class="whitespace-nowrap rounded-radius bg-transparent p-0.5 hover:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 active:opacity-100 active:outline-offset-0">
